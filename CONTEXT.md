...
**D.VA 第一阶段课程收敛**:
`examples/train_dva_avoidance.py` 在短程动态避障课程中使用 goal-directed warm start 得到真实 `CNNLidarActor` 参数，再运行 D.VA rollout 和 privileged critic 训练验证路径。当前 actor 在 D.VA 阶段冻结，用闭环评估证明课程内成功率，而不声明解析 actor-gradient 微调已经稳定。
_避免_: 把 actor 冻结的课程收敛结论推广到完整动态避障任务或解析 LiDAR 梯度训练。

**D.VA actor-visible schema**:
D.VA actor 仍接收 226 维观测，但 `dynamic_avoidance_dva_adapter` 会把 LiDAR 归一化，并把目标方向、速度转换到 body frame，同时加入高度和上一时刻 body rates。该 schema 面向低层 body-rate action 语义。
_避免_: 直接把环境原始 flat observation 当作 D.VA actor 的训练/评估输入。

**D.VA privileged critic schema**:
D.VA critic 使用由 `DynamicAvoidanceEnvState` 派生的 127 维 privileged observation，包括 ego state、goal state、clearance field、clearance delta、TTC、last action 和 progress。当前课程收敛变更未改变 critic 维度和语义。
_避免_: 把 actor-visible schema 调整理解为 privileged critic schema 调整。
...
- **D.VA 第一阶段课程收敛**是当前可复现实验结论，依赖短程动态避障课程、goal-directed warm start、冻结 actor 的 D.VA critic validation 和多 seed policy evaluation。
...
- D.VA 动态避障当前已完成 headless smoke validation，并在第一阶段短程动态课程中达到真实 actor rollout 验收：`examples/train_dva_avoidance.py` 固定 seeds `(123, 124, 125, 126, 127)` 的 success rate 为 `0.80`、termination rate 为 `0.20`，seed `123` 到达 `2.995 m`。完整动态避障任务、解析 LiDAR 梯度 actor 更新、reward 分量日志和 unfrozen actor-gradient 微调仍是后续工作。
